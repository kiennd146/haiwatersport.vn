...
function gk_is_music_anim(wrapper, contents, dates, slides, which, $G){	
	if(which != $G['actual_slide']){
		...
		(function() {
			new Fx.Tween($G['pages_content'][actual], {property: 'opacity', duration: $G['anim_speed'] / 2}).start(1,0);
		...
		$G['pages_content'][actual].getElements('.gkIsSlide').each(function(el,i) {
			(function() {
				...
				new Fx.Tween(el, { duration: $G['anim_speed'] / 2 }).start('opacity', 0);
		...
		(function() {
			...
			new Fx.Tween($G['pages_content'][which], {property: 'opacity', duration: $G['anim_speed'] / 2 }).start(0,1);
			
			$G['pages_content'][which].getElements('.gkIsSlide').each(function(el,i) {
				(function() {
					new Fx.Tween(el, { duration: $G['anim_speed'] / 2 }).start('margin-top', 0);
					new Fx.Tween(el, { duration: $G['anim_speed'] / 2 }).start('opacity', 1);
