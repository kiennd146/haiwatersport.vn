<?php
/**
 * @version		$Id: view.raw.php 1276 2011-10-27 16:23:08Z lefteris.kavadas $
 * @package		K2
		...
		switch ($task) {
			...
			case 'category':
				...
				if (! empty($category->image)) {
					$category->image = JURI::root(true).'/media/k2/categories/'.$category->image;
				} else {
					if ($params->get('catImageDefault')) {
						$category->image = JURI::root(true).'/components/com_k2/images/placeholder/category.png';
					}
				}
				...
				if (count($children)) {
					foreach ($children as $child) {
						...
						if (! empty($child->image)) {
							$child->image = JURI::root(true).'/media/k2/categories/'.$child->image;
						} else {
							if ($params->get('catImageDefault')) {
								$child->image = JURI::root(true).'/components/com_k2/images/placeholder/category.png';
							}
						}
			...
			case 'date':
				...
				if (JRequest::getInt('day')) {
					...
					$title = JText::_('K2_ITEMS_FILTERED_BY_DATE').' '.JHTML::_('date', $date, '%A, %d %B %Y');
				} else {
					$date = strtotime(JRequest::getInt('year').'-'.JRequest::getInt('month'));
					$title = JText::_('K2_ITEMS_FILTERED_BY_DATE').' '.JHTML::_('date', $date, '%B %Y');
				}
				break;
